...
const Home = ({ setAdd }) => {
  ...
  useEffect(() => {
    // Save cart to sessionStorage whenever it is updated
  ...
  const removeFromCart = (index) => {
    ...
    setCart(updatedCart);
    setAdd((prevAdd) => Math.max(prevAdd - 1, 0)); // Decrement the add count
  };
  ...
  const updateQuantity = (index, delta) => {
    const updatedCart = cart.map((item, i) => {
      if (i === index) {
        const updatedItem = {
          ...item,
          quantity: Math.max((item.quantity || 1) + delta, 1), // Ensure quantity doesn't go below 1
        };
        return updatedItem;
      }
      return item;
